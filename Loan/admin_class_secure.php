...
class SecureAction {
    ...
    public function save_loan() {
        ...
        if (empty($id)) {
            ...
            $stmt = $this->conn->prepare("INSERT INTO loan_list (ref_no, borrower_id, loan_type_id, plan_id, amount, purpose) VALUES (?, ?, ?, ?, ?, ?)");
            $stmt->bind_param("siiids", $ref_no, $borrower_id, $loan_type_id, $plan_id, $amount, $purpose);
            
        } else {
            if ($status !== null) {
                $stmt = $this->conn->prepare("UPDATE loan_list SET borrower_id = ?, loan_type_id = ?, plan_id = ?, amount = ?, purpose = ?, status = ? WHERE id = ?");
                $stmt->bind_param("iiiidsi", $borrower_id, $loan_type_id, $plan_id, $amount, $purpose, $status, $id);
                ...
                $stmt = $this->conn->prepare("UPDATE loan_list SET borrower_id = ?, loan_type_id = ?, plan_id = ?, amount = ?, purpose = ? WHERE id = ?");
                $stmt->bind_param("iiiidi", $borrower_id, $loan_type_id, $plan_id, $amount, $purpose, $id);
            }
        }
