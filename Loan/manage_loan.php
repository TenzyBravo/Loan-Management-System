...
<div class="container-fluid">
	...
	<form action="" id="loan-application">
		...
		<input type="number" name="amount" class="form-control text-right" step="any" id="loan_amount_input" value="<?php echo isset($amount) ? $amount : '' ?>">
		</div>
		<div class="form-group col-md-2">
		<label class="control-label">Duration (months)</label>
		<input type="number" name="duration_months" class="form-control text-right" step="1" min="1" id="duration_months" value="<?php echo isset($duration_months) ? $duration_months : (isset($plan_id) && $plan_id ? $conn->query("SELECT months FROM loan_plan WHERE id = " . intval($plan_id))->fetch_assoc()['months'] ?? 1 : 1) ?>">
		</div>
		<div class="form-group col-md-6">
		<label class="control-label">Interest Rate</label>
		<select name="interest_rate" class="form-control" id="interest_rate_select" required>
		  <option value="10" <?php echo (isset($interest_rate) && $interest_rate == 10) ? 'selected' : '' ?>>10%</option>
		  <option value="18" <?php echo (isset($interest_rate) && $interest_rate == 18) ? 'selected' : '' ?>>18%</option>
		  <option value="25" <?php echo (isset($interest_rate) && $interest_rate == 25) ? 'selected' : '' ?>>25%</option>
		  <option value="28" <?php echo (isset($interest_rate) && $interest_rate == 28) ? 'selected' : '' ?>>28%</option>
		  <option value="30" <?php echo (isset($interest_rate) && $interest_rate == 30) ? 'selected' : '' ?>>30%</option>
		  <option value="35" <?php echo (isset($interest_rate) && $interest_rate == 35) ? 'selected' : '' ?>>35%</option>
		  <option value="40" <?php echo (isset($interest_rate) && $interest_rate == 40) ? 'selected' : '' ?>>40%</option>
		</select>
		</div>
		<div class="form-group col-md-6">
		<label class="control-label">Calculation</label>
		<select name="calculation_type" class="form-control" id="calculation_type_select" required>
		  <option value="simple" <?php echo (isset($calculation_type) && $calculation_type == 'simple') ? 'selected' : '' ?>>Simple</option>
		  <option value="compound" <?php echo (isset($calculation_type) && $calculation_type == 'compound') ? 'selected' : '' ?>>Compound</option>
		</select>
		</div>
		</div>
		<div class="row">
...
<script>
	...
	function calculate(){
		start_load()
		var amount = $('#loan_amount_input').val();
		if($('#plan_id').val() == '' && (!amount || amount <= 0)){
			alert_toast("Select plan and enter amount first.","warning");
			return false;
		}
		var plan = $("#plan_id option[value='"+$("#plan_id").val()+"']");
		var months = parseInt($('#duration_months').val()) || (plan.attr('data-months') ? parseInt(plan.attr('data-months')) : 1);
		var interest = $('#interest_rate_select').val() || (plan.attr('data-interest_percentage') ? plan.attr('data-interest_percentage') : 18);
		var calc_type = $('#calculation_type_select').val() || 'simple';
		var penalty = plan.attr('data-penalty_rate') || 0;
		$.ajax({
			url:"calculation_table.php",
			method:"POST",
			data:{amount:amount, months:months, interest_rate:interest, calculation_type:calc_type, penalty:penalty},
	...
	// When plan changes, set default months and interest
	$('#plan_id').on('change', function(){
		var plan = $("#plan_id option[value='"+$(this).val()+"']");
		if(plan.length){
			$('#duration_months').val(plan.attr('data-months'));
			// set interest dropdown to plan interest if available
			if(plan.attr('data-interest_percentage')){
				$('#interest_rate_select').val(plan.attr('data-interest_percentage'));
			}
		}
	});
